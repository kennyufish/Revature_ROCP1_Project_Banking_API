	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			...
			if(isSuccess) {
				...
				String convertedBalance = n.format(Double.parseDouble(request.getParameter("withdrawAmount")));
				//
				...
				out.print("UNSUCCESSFUL WITHDRAW - PLEASE TRY AGAIN"+"<h1></div>");
			}
			...
			out.print("<div><h1 class='topNoticeWarning'>*The requested action is not permitted from withdrawController*<h1></div>");
			requestDispatcher.include(request, response);
